...
class DealRecord(object):
    def __init__(self):
        ...
        self.phone_pattern = re.compile(r'^1[34578]\d{9}')
    ...
    def deal_text(self, content: list):
        time_pattern = re.compile(
            r'([0-9]{3}[1-9]|[0-9]{2}[1-9][0-9]{1}|[0-9]{1}[1-9][0-9]{2}|[1-9][0-9]{3})-(((0[13578]|1[02])-(0[1-9]|[12][0-9]|3[01]))|((0[469]|11)-(0[1-9]|[12][0-9]|30))|(02-(0[1-9]|[1][0-9]|2[0-9]))) (0[0-9]|1[0-9]|2[0-3]):([0-5][0-9]):([0-5][0-9])'
        )
        if len(content) <= 1:
            return
        ...
        for item in content:
            if '----------------------------' in item:
                ...
                sign = re.search(time_pattern, item)
    ...
    def starting(self, content):
        count = 0
        for item in content:
            if count > 5:
                break
            elif item['person'] != self.info['客户ID']:
                if item['text'] not in self.starting_list:
                    self.statistical['未按开头话术回复'].append(item['person'])
                    break
    ...
    def get_phone(self, content):
        phone_obj = re.search(self.phone_pattern, content)
    ...
    def deal_time(self, content: list):
        ...
        for _ in content[::-1]:
            if _['person'] != self.info['客户ID']:
                last_ = content.index(_)
                break

        for index, item in enumerate(content):
            if item['person'] == self.staff:
                if sign == 0 and record_time is not None and index < last_:
    ...
    def extract_record(self, path):
        ...
        text = re.findall(r'={64}.*?={64}(.*?)={64}', content,
                          re.S)[0].strip('\n').split('\n')
        ...
        for key, value in text_list.items():
            ...
            if self.statistical[shop].get(self.staff.split(':')[-1]) is None:
                self.statistical[shop][self.staff.split(':')[-1]] = 0

            self.statistical[shop][self.staff.split(':')[-1]] += 1
    ...
    def summary(self, path):
        ...
        try:
            ...
            shutil.copyfile(path, summary_path)
        else:
    ...
    def single(self):
        ...
        d = self.statistical.pop('未按开头话术回复')
        ...
        self.excel.init_sheet()
        self.excel.write(['未按开头话术回复的客服'])
        d = set(d)
        for person in d:
            self.excel.write([person])
        self.excel.save('./data/结果/未按开头话术回复的客服.xlsx')

        try:
            last_data = self.excel.read('./data/结果/有关行业.xlsx')
        except FileNotFoundError:
            self.excel.init_sheet()
            self.write(
            ...
            self.excel.save(path='./data/结果/有关行业.xlsx')
        else:
            self.excel.append([])
            self.excel.append(
                [time.strftime('%Y.%m.%d', time.localtime(time.time()))])
            self.excel.append(['有关行业'])
            for item in c.items():
                self.excel.append(item)
            self.excel.save(path='./data/结果/有关行业.xlsx', mode='read')

    def main(self):
        self.some_input()
        self.statistical = {'一句后再无回复': 0}
        self.statistical['未按开头话术回复'] = []
        ...
        path = '{}开头话术.txt'.format(self.ending_path)
        self.starting_list = read_lines(path, judge_code(path))
