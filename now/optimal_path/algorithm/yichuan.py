...
class GeneticAlgorithm:
    ...
    d = [
        [0, 4, 6, 7.5, 9, 20, 10, 16, 8],  #配送中心（编号为0）到8个客户送货点的距离
    ...
    def calFitness(self, line, isShow):
        ...
        for i in range(0, self.mans):
            ...
            if newDis + self.d[
                    line[i]][0] > self.distance or newTon > self.tons:
                #下一辆车
                totalDis += carDis + self.d[fore][
                    0]  #后面加这个d[fore][0]表示需要从当前客户处返程的距离
                r += 1
                fore = 0
                i -= 1  #表示当前这个点的配送还没有完成
                carTon = 0
