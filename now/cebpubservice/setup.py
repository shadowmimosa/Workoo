...
def getpy(basepath=os.path.abspath('.'),
    ...
    for fname in os.listdir(fullpath):
        ...
        if os.path.isdir(
                ffile) and fname != build_dir and not fname.startswith('.'):
            ...
            if ext == ".c":
                ...
                if os.path.splitext(fname)[1] in (
                        '.py', '.pyx') and not fname.startswith('__'):
                    yield os.path.join(parentpath, name, fname)
                elif copyOther:
                    dstdir = os.path.join(basepath, build_dir, parentpath,
                                          name)
...
print("complate! time:" + str(time.time() - starttime) + 's')
