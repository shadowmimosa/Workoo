...
class DealShfe(object):
    ...
    def get_date(self):
        uri = 'http://www.shfe.com.cn/js/trade-data.js?200609224937'
        resp = requests.get(uri, headers=self.header)
        result = re.search(r"return ({.*'});", resp.text)
        ...
        for year in date_string.split("',"):
            dates = year.replace("'", '').split(':')[-1]
            date_list.extend(dates.split(',')[::-1])

        for date in date_list:
            yield date

    def run(self, date):
        # uri = f'http://www.shfe.com.cn/data/dailydata/kx/pm{next(date)}.dat'
    ...
    def main(self):
        date = get_date([x for x in range(2019, 2021)])
        while True:
            try:
                now = next(date)
