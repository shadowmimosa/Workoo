...
class DealZcy(object):
    ...
    def deal_detail(self, path):
        ...
        inquiry_detail = run_func(self.soup,
                                  resp,
                                  attr={"class": "inquiry-detail-container"})

        inquiry_info = run_func(self.soup,
                                run_func(self.soup,
                                         inquiry_detail,
                                         attr={"class": "inquiry-info"}),
                                attr="section",
                                all_tag=True)

        inquiry_base = run_func(self.soup,
                                run_func(self.soup,
                                         inquiry_info[0],
                                         attr="table"),
                                attr="tr",
        ...
        info = {}
        info["网上竞价名称"] = run_func(self.soup,
                                  run_func(self.soup,
                                           inquiry_detail,
                                           attr={"class": "inquiry-title"}),
                                  attr="h1").text

        info["网上竞价编号"] = inquiry_base[1].text.strip('\n').split('\n')[1]

        info_temp = inquiry_base[2].text.strip('\n').split('\n')

        info["竞价开始时间"] = info_temp[1]
        info["竞价截止时间"] = info_temp[3]
        info["采购人"] = info_temp[5]
        info["金额上限"] = inquiry_base[4].text.strip('\n').split('\n')[-1]
        info["path"] = path

        goods_list = run_func(self.soup,
                              inquiry_info[2],
                              attr="table",
                              all_tag=True)
        tr_json = []
        ...
        for index, good_obj in enumerate(goods_list):
            ...
            result = re.search(r'[\u4e00-\u9fa5]+', good[sign + 2].text)
            unit = result.group() if result else ''
            tr_json.append({
                "参数": param,
                "单位": unit,
                "招标编号": info["网上竞价编号"],
    ...
    def deal_result(self, path):
        ...
        inquiry_detail = run_func(self.soup,
                                  resp,
                                  attr={"class": "inquiry-result-container"})
    ...
    def run(self, data):
        ...
        for item in data["result"]["data"]:
            if run_func(self.judge_already, item["orderId"]):
                ...
                elif self.bid_type == "RESULT" and item["stateStr"] == "询价完成":
