...
def main():
    ...
    for href, current in href_list:
        team_id = run_func(match_result, href)
        if not team_id:
            continue
        teams.append(team_id) if team_id not in teams else True

        if team_id:
            break

    for local_team in teams:
        result = run_func(summary_team, local_team, current)
