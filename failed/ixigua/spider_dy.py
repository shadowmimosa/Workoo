...
def make_it():
    ...
    try:
        ...
        #视频名字
        video_name = re.findall('<p class="name nowrap">(.*?)</p>', text_data,
                                re.S)[0]
        #视频简介
        video_summary = re.findall('<p class="desc">(.*?)</p>', text_data,
                                   re.S)[0]
        #视频封面
